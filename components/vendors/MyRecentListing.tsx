...
const MyRecentListing = () => {
    ...
    const submitModal = (updatedProduct) => {
        const updatedList = listingsData.map(item => item.id === updatedProduct.id ? updatedProduct : item);
        setListingsData(updatedList);
        closeModal();
    };

    const renderItem = ({ item }) => {
        const circleColor = item.status.toLowerCase() === 'in stock' ? '#2E7D32' : '#FFD700';
    ...
    if (loading) return (
        <ActivityIndicator size="large" color="rgba(255,202,40,1)" style={{ flex:1, justifyContent:'center', alignItems:'center' }} />
    );
...
const styles = StyleSheet.create({
    ...
    headerRowContainer: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 16, marginBottom: 12 },
    headerTitle: { fontSize: 18, fontWeight: '700', color: '#333' },
    seeAll: { fontSize: 13, color: '#0AA1FF' },
    flatListContent: { paddingHorizontal: 16 },
    listingCard: { backgroundColor: '#fff', borderRadius: 8, marginRight: 15, borderWidth: 1, borderColor: 'rgba(255, 202, 40, 1)', width: Dimensions.get("window").width * 0.8 },
    cardContent: { flexDirection: 'row', alignItems: 'center', gap: 12 },
    imageContainer: { width: 120, height: 140 },
    itemImage: { width: '100%', height: '100%', borderTopRightRadius: 0, borderBottomRightRadius: 0, borderTopLeftRadius: 6, borderBottomLeftRadius: 6 },
    textContainer: { flex: 1, paddingRight: 12, paddingVertical: 8 },
    headerRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 },
    itemName: { fontSize: 16, fontWeight: '600', color: 'rgba(66, 66, 66, 1)', flex: 1, marginRight: 8 },
    priceQuantityContainer: { alignItems: 'flex-end', position:'absolute', marginLeft:145 },
    ...
    startAllIndia: { flexDirection:'row', alignItems:'center', gap:5, marginVertical: 4 },
    txetAll: { fontSize:13 },
    editBtn: { flexDirection: 'row', justifyContent: "space-between", alignItems: 'center', marginTop: 8 },
    dropdownBtn: { padding:4, borderRadius:8, borderWidth:1, borderColor:'rgba(0, 0, 0, 0.3)' },
    statusRow: { flexDirection: 'row', alignItems: 'center', gap: 6 },
    statusCircle: { width: 10, height: 10, borderRadius: 5 },
    statusText: { fontSize: 14, fontWeight: '500', color: 'rgba(255, 202, 40, 1)' },
